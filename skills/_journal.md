...
## 2026-06-24 — folded omas-odoo's nitpick review of the leasing-PDC PR (psae-mcy #1) + line-by-line mentality

**What:** I gave an initial "approve-with-changes, mostly solid" review; omas-odoo's actual `CHANGES_REQUESTED` pass + a long chat drill-down surfaced ~9 nitpicks I'd waved through. Verbatim reviewer phrases:
- "putting it in an internal function doesn't make it any better" / "hiding the search loop in the internal function doesn't make it better" (product_category:64, sale_order:45) — a per-row `create`/`search` extracted into a private helper, presented as if that fixed it.
- `_unit_contract_price` — "`unit_lines[0]` → `sum(mapped)`", "do we need the `or`, seems dead", "the `if` has a dead condition too", "the comment seems useless".
- `_compute_property_status` — "`if`/`else` overrides all values", "3 fields duplicated in the elif", "this can be removed" (same field assigned in every branch + after; the stage `search` could sit outside the loop).
- "this could be done in the domain of the query" (a `for: if cond: continue` straight after a `search`, check_incoming:325).
- cron: "should start with `_cron`, public = anyone can rpc call it", "run twice → posts twice".
- "just have 2 constrains … internal function to avoid duplicated code" (res_partner:21).
- review summary: "no point making an AI write an essay as a commit message".
Then two convention asks: never emit the `<?xml ...?>` prolog; don't add a `string=` that just restates the field's derived label. And the deeper calibration: "code review like me, line by line, not just a full overview."

**Why it matters:** My review calibration catches blockers/security but waves through the nitpick class a senior gates on. Sharpest new shape: **extraction-as-camouflage** — moving a per-row op into a private helper doesn't pay the debt; the smell is the per-row op, not where it lives. But the *root cause* is granularity: a thematic overview never reaches the nitpick on line 237. Fixing the shapes without fixing the read would just miss them more confidently.

**Likely target skill:** odoo-code-review (the line-by-line read + dead-code list + .py shapes) + odoo-python (loop-is-the-smell, Security/cron) + odoo-task-completion (terse commits) + odoo-xml-conventions (prolog, redundant string). NO-OP: `<data noupdate>` (covered), `_(...) % x` translation (python-conventions L163), blank-line grouping (L187), `_get_records_action` (orm-patterns) — application misses, not content gaps.
**Status:** [done] processed 2026-06-24 — odoo-code-review: reframed "Read it twice" → lean "Two passes — don't mix them" (Pass 1 file by file *and* line by line, nitpick as the senior reviewer, anchor every finding to `file:line`, thematic summary named as the failure mode; Pass 2 the system trace) — user preferred the explicit two-phase format but leaner; extended dead-code list (dead `or`/`if`/restating-comment/duplicated-branch-assign); .py-shapes (`filtered(...)[0]` + helper-tucked loop). odoo-python: sharpened "loop is the smell" (extraction-camouflage + post-search filter→domain + hoist loop-invariant) and "Whose rights" (cron `_cron_`/private/idempotent). odoo-task-completion: commit/PR terse + human. odoo-xml-conventions/xml-format.md: "No XML prolog" + "Don't restate the field name" sections + 2 checklist items.
